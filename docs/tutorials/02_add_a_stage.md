...
`stage.py` — the `rainspout.contracts` import is the framework; `typing` and
`pydantic` supply the bounds on *this stage's* knobs:
...
Add to `components.py` (again with the linter guard — PACKAGE_AUTHORING §4):

```python
from my_package.stages.smooth_readings import stage as _  # noqa: F401
```
...
components: smooth_readings ✓  readings_local_csv ✓
4 passed
```
...
`version` in `stage.py`: the CI version-bump check fails, naming the
`stages/smooth_readings` directory. Bump to `1.0.1`; it passes. Editing only
`test_smooth_readings.py` requires no bump — tests are outside the code-hash
boundary. (No PR handy? The check runs locally too:
`python -m rainspout.devtools.version_bump --base main` — it diffs
*committed* changes against the base ref, exactly as CI will.)
