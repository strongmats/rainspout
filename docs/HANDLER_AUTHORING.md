...
A handler is the one piece of code that knows how a particular kind of data
is stored — the file format, the folder layout, the naming scheme, how to
reach it — so nothing else has to. You write a small class with three private
methods: read one cell of data (`_load_cell`), write one (`_save_cell`), and
list which cells exist (`_catalog_cells`). Up front you declare what the
handler needs to reach its data (its *resources*, e.g. a base folder) and
which axes its storage is organized along (its *roles*, e.g. "a day and a
sensor"). Rainspout wraps your three methods with everything else:
validation, iterating over ranges, error reporting. Give the class a name,
ship a tiny example data file and one round-trip test, and it's usable from
any run config.

The rest of this document is the precise contract. New here? Skim §1–§4,
build [Tutorial 1](tutorials/01_add_a_handler.md), and come back for details
as you need them.
...
## 3. The shape of a handler — template first, then a worked example
...
# yours: every other import (file-format library, date types, API client)
# belongs to YOUR storage domain — the framework mandates none of them


class MyHandlerResources(HandlerResources):   # yours: the name; contract: the base class
    """What this handler needs to reach its data."""
    ...                                       # yours: bounded fields (§5) — a folder, a URL, a key


class MyHandler(Handler):                     # contract: subclassing Handler IS registration
    name = "my_handler"                       # contract: attribute must exist; yours: the value (§2)
    resources_model = MyHandlerResources      # contract: must point at your resources model
    dimension_roles = ("role_a", "role_b")    # contract: attribute; yours: your storage's axes (§6)
    dimension_types = {"role_a": ..., "role_b": ...}   # contract: one Python type per role
    supports_grid_range = False               # contract: flag exists; yours: set honestly (§7.1)
    supports_windowed_read = False            # contract: flag exists; yours: set honestly (§7.2)

    def _load_cell(self, coords: Coords) -> tuple[object, Meta]:
        ...                                   # yours: read ONE cell → (data, metadata) (§4, §11)

    def _save_cell(self, coords: Coords, data: object, meta: Meta) -> None:
        ...                                   # yours: write ONE cell, metadata embedded (§4, §11)

    def _catalog_cells(self, spec):
        ...                                   # yours: yield CatalogEntry per existing cell (§9)
```

Line by line:

- **The `rainspout.contracts` import** is the entire framework surface a
  handler touches: the base classes (`Handler`, `HandlerResources`) and the
  types that cross the boundary (`Coords` — a mapping of role name → one
  value; `Meta` — the metadata block, §11; `CatalogEntry` — one "this cell
  exists" record, §9). Anything else you import serves your format, not the
  contract.
- **The resources model** declares, as typed fields, everything the handler
  needs to *reach* its data. The skeleton validates a config's `resources:`
  block against it at startup — unknown fields, missing fields, and
  out-of-range values all fail loudly, naming the field (§5). After
  construction it's available read-only as `self.resources`.
- **`name`** is the registry key config files use to refer to your handler
  class. Purely a label — never parsed for meaning (§2).
- **`resources_model`** wires your resources class to the base `__init__`
  (which you never write — §below) so validation is un-bypassable.
- **`dimension_roles`** are *your* names for the axes your storage layout
  resolves — declared here because you're written before any config exists;
  config authors map their dimension names onto your roles (§6).
- **`dimension_types`** gives one Python type per role, so the skeleton can
  check at startup that a run's dimension values will make sense to you.
- **The two capability flags** declare what your storage genuinely supports:
  multi-cell load specs (§7.1) and partial reads within a cell (§7.2). Both
  default to off; claiming one falsely is a conformance failure.
- **The three hooks** are the whole job: each reads, writes, or lists
  **one cell at a time**. You never call them yourself — the skeleton's
  public verbs (`load`, `save`, `catalog`, §4) wrap them with spec
  validation, lazy iteration, and error messages that name you and the
  offending coordinate.

**Worked example** — the same shape filled in for one concrete domain: daily
sensor readings, one local CSV file per (day, sensor). The `date` import,
the `day`/`sensor` roles, and `DirectoryPath` are all this *example's*
choices; swap them for your own:

```python
from datetime import date                  # example's domain: its cells are keyed by calendar day
...
class ReadingsLocalCsvResources(HandlerResources):
    ...
    base_dir: DirectoryPath                # this handler reads a local folder tree


class ReadingsLocalCsv(Handler):
    name = "readings_local_csv"
    resources_model = ReadingsLocalCsvResources
    dimension_roles = ("day", "sensor")    # this storage is organized by day, then sensor
...
Tutorial 1 builds this exact handler with full method bodies.
...
For callers, the base also provides **`load_one(coords)`** — final, exactly
`load` with a one-value spec, returning `(data, meta)` directly instead of an
iterator. It exists because a stage calling an auxiliary handler almost always
wants one cell per call (§6); you implement nothing extra for it.
...
- **Auxiliary handler** — handed to a stage as a `handler:`-wired dependency
  (events files, calibration tables, artifacts). The **stage** computes the
  coordinates it asks you for — typically derived from its work-item
  coordinate, along axes that may have nothing to do with the run's
  dimensions — and calls your public verbs directly (`load_one`, or `load`
  in a loop). It is **not dimension-validated at startup**: the skeleton
  guarantees the stage only that you exist and your resources validate;
  whether what you serve fits the stage's science — including legitimate
  superset/subset relationships, where you load a rich structure and the
  stage uses one piece — proves out at runtime, and a genuine mismatch is an
  isolated, logged, per-work-item failure, never a startup one.

**Worked example — the auxiliary pattern, seen from the stage side.** The run
iterates `day × sensor`; the auxiliary handler indexes weather events by
`(lat, lon, hour)` — unrelated axes, which is the *normal* case. The stage
derives the coordinates it needs and calls the handler it was handed:

```python
class DetectEventsDependencies(StageDependencies):
    data: LazyReference          # from: an upstream stage
    events: Handler              # handler: events_in

class DetectEvents(Stage):
    ...
    def run(self, deps):
        readings = deps.data.get()
        day = deps.data.coords[self.settings.day_dim]
        site = SITE_LOCATIONS[deps.data.coords[self.settings.sensor_dim]]
        events = []
        for hour in range(24):                    # a "window" is just a loop
            data, _meta = deps.events.load_one(
                {"lat": site.lat, "lon": site.lon, "hour": hour_of(day, hour)}
            )
            events.extend(data)
        return detect(readings, events, self.settings.threshold)
```

The skeleton played no part in those coordinates: it guaranteed the stage a
validly-configured handler instance and nothing more. A wrong role name or an
absent cell fails this one work item at runtime, logged, and the run
continues.
...
multi-cell `load` spec — at startup pre-flight when the ask comes from the
run's own wiring (seed position), or at call time, failing that work item
only, when a stage asks (stages can always fall back to `load_one` in a
loop, which works against every handler). `catalog` always accepts ranges
regardless — surveying windows is its job.
...
runs the shipped helper. The three ALL-CAPS names and the use of
`assert_roundtrip` are the contract; every value is the example's:

```python
from pathlib import Path                      # yours: however you locate your example data
from rainspout.testing import assert_roundtrip   # contract: the shipped helper
from .handler import ReadingsLocalCsv

HANDLER = ReadingsLocalCsv                    # contract: this exact name; yours: the class
EXAMPLE_RESOURCES = {"base_dir": Path(__file__).parent / "example_data"}
                                              # contract: the name; yours: valid resources
EXAMPLE_COORDS = {"day": "2026-01-01", "sensor": "s1"}
                                              # contract: the name; yours: one real example cell,
                                              #   keyed by YOUR roles

def test_roundtrip(tmp_path):                 # contract: at least one test_ function
...
in a comment. The save side of the trip is constructed with your
`EXAMPLE_RESOURCES` retargeted to the temporary directory via the `base_dir`
field; if your resources have no `base_dir`, pass `save_resources=` explicitly.
