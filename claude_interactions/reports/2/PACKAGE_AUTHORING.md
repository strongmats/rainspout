...
Two entry-point groups, declared in `pyproject.toml`:
...
2. The consuming stage declares a `BoundHandler` dependency and loads the
   artifact at run time like any input — mid-DAG extra dependencies are normal
   (STAGE_AUTHORING §5).
