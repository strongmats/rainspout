...
- **Naming convention:** `datatype_channel_type` (e.g. `broadband_local_mat`,
  `cleaned_local_h5`) — underscores, no dot, because handlers are Python module files
  (`broadband_local_mat.py`). Purely conventional — never parsed by the skeleton for
  meaning. It is a registry key; capabilities are declared explicitly in code.
...
- **Three strictly separated kinds of input:**
  - **Settings** — static config, deep-validated by Pydantic (`settings_model`,
    `extra="forbid"`, and a BOUNDED valid domain for every setting — numeric ranges
    via `Field`, discrete choices via `Literal`/`Enum`, constrained strings, etc.
    Unbounded settings are permitted only as a deliberately justified exception, never
    the default. Cheap to validate, so validated hard: out-of-range or invalid-option
...
- **No side effects, no fetching outside declared dependencies, no calling other
  stages.**
- **One stage = one self-contained directory** — a self-contained unit (NOT one file)
  that can and generally should have internal substructure: the thin stage class, its
  module-level science functions, helpers, its test, any stage-local resources — organized
  however suits the stage (self-containment and not-one-file is the point, not flatness).
  The version-bump CI check and provenance code-hash watch the stage CODE file(s) only, so
  editing the test does not force a version bump.
...
- **Startup pre-flight check (handler ↔ dimension sanity).** Beyond checking that
  config dimension names line up with handler-declared dimension roles (definition-time),
  the driver asks each handler at startup to validate a SINGLE probe coordinate —
  cheaply confirming the dimension spec resolves to structurally-sane data (right
  type/structure) without surveying all data. A mismatch (even with correct names but a
  type/structure incompatibility) fails loudly immediately. Genuine per-coordinate data
  problems still surface per-work-item at runtime. There is deliberately NO inter-stage
  structural compatibility check — validating stage→stage data structure would require
  granular per-input/output contracts (too heavy an authoring burden); a wrong structure
  handed between stages surfaces as a normal isolated per-work-item runtime failure.
  Validate the edges (data in from storage) strictly; do not burden internal
  stage-to-stage seams.
- **Distinct failure modes, each with a named-offender message:** bad setting (incl.
  out-of-range/invalid-option); missing/
  ...
  handler; dimension/data structural mismatch at pre-flight; a changed stage that didn't
  bump its version.
...
- **Tests (in the skeleton repo `tests/`)** prove the MACHINERY works — minimal,
  ...
  **mid-DAG extra dependency** (a stage that loads an additional input partway
  through, via a handler) AND a **trivial, observable setup-hook exercise** (e.g. the
  hook writes a marker / sets a flag) to prove the setup hook fires. (No setup-time
  compilation / Cython requirement — deliberately dropped to keep CI simple; a C
  toolchain is not pulled into CI.)
...
- Content-package territory (not skeleton): **h5py / scipy** (HDF5 / .mat),
  **ONNX Runtime** (if a package does inference),
