...
| Live status without threads: `set_status`/`set_progress` calls tick **one in-place line** (`\r`-redrawn on stderr, cleared before permanent output) in the terminal running `spout run`; `--live/--no-live` overrides the stderr-TTY autodetect; best-effort by contract. Replaces the v1.1.0 status file + `spout status` command — status lives where the run was launched, no files | v1.2.0 |
| One active run per run definition (config location + `run.name`): flock-based lock file next to the oplog; a second concurrent `spout run` fails at startup naming the holder (pid, run_id); concurrency = distinct `run.name`s | v1.1.0 |
