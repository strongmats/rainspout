...
| Live status without threads: `set_status`/`set_progress` calls tick a throttled, atomically-written status file next to the oplog (`<name>.status.json`); `spout status --config` reads it from any terminal; best-effort by contract | v1.1.0 |
